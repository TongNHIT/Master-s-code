...
main(int argc, char** argv)
{
	...
	pcl::io::loadPCDFile("e:/Pcl_Program/pcdData/rabbit_trans.pcd", *cloud_src_o);
	PointCloud::Ptr cloud_tgt_o(new PointCloud);//目标点云
	pcl::io::loadPCDFile("e:/Pcl_Program/pcdData/rabbit_t.pcd", *cloud_tgt_o);
	...
	///clock_t start2 = clock();   //FPFH特征计算结束
	...
	///cout << "法线估计时间: " << (double)(start1 - start) / (double)CLOCKS_PER_SEC << " s" << endl;  //法线估计时间
	///cout << "FPFH特征计算时间: " << (double)(start2 - start) / (double)CLOCKS_PER_SEC << " s" << endl;  //FPFH特征计算时间
